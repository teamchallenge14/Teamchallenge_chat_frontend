...
export const SocialAuth = () => {
  const handleOAuthLogin = (provider: OAuthProvider) => {
    // 1. Берем базовый URL. Убедись, что в .env он БЕЗ слэша на конце.
    const baseUrl = import.meta.env.VITE_API_URL;
    
    if (!baseUrl) {
      console.error("Ошибка: VITE_API_URL не определен в .env файле");
      return;
    }

    // 2. Формируем финальный путь. 
    // Для твоего бэкенда это должно быть: baseUrl + /auth/ + provider
