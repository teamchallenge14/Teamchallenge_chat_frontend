...
export const SocialAuth = () => {
 const handleOAuthLogin = (provider: OAuthProvider) => {
  ...
  // Если baseUrl не подтянулся из .env, используем прямую строку
  const finalBaseUrl = baseUrl ? baseUrl : 'https://teamchallenge-chat-backend.onrender.com/v1';
  
  // Собираем финальный адрес
  const authUrl = `${finalBaseUrl}/auth/${provider}`;
